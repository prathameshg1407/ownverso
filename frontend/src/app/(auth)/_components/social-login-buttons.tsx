// ==== FILE: src/app/(auth)/_components/social-login-buttons.tsx ====
/**
...
export function SocialLoginButtons({
  ...
  const handleSocialLogin = useCallback(
    async (providerId: AuthProvider) => {
      ...
      try {
        const { authUrl } = await oauthApi.initOAuth(providerId);
        window.location.replace(authUrl);
      } catch (error) {
        toast.error(formatError(error).message || `Failed to connect with ${providerId}`);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Buttons */}
      <div
        className={layout === 'horizontal' ? 'grid grid-cols-3 gap-3' : 'flex flex-col gap-2'}
      >
        {PROVIDERS.map((provider) => (
          <Button
            ...
            type="button"
            variant="outline"
            size={layout === 'horizontal' ? 'icon' : 'default'}
