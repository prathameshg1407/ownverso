...
import { isUpgrade } from '../entities/author-account.entity';
...
const PLATFORM_PLANS: PlatformPlanDTO[] = [
  {
    tier: 'FREE',
    name: 'Free',
...
export const authorPlatformSubService = {
  /**
   * Get all available platform plans
   */
  async listPlans(): Promise<PlatformPlanDTO[]> {
   ...
   */
  async getPlan(tier: PlatformTier): Promise<PlatformPlanDTO> {
    const plan = PLATFORM_PLANS.find((p) => p.tier === tier);
    if (!plan) {
      throw new NotFoundError('Plan not found', ERROR_CODES.NOT_FOUND);
    }
    return plan;
  },
   ...
   */
  async getSubscription(userId: bigint): Promise<PlatformSubscriptionDTO> {
    ...
    return authorAccountMapper.toPlatformSubscriptionDTO(account);
  },
  ...
  async subscribe(
    ...
    if (account.platformTier !== 'FREE') {
      throw new BadRequestError(
        'Already subscribed to a plan. Use change plan instead.',
    ...
    const updated = await authorAccountRepository.updatePlatformSubscription(
      userId,
      {
        platformTier: tier,
    ...
    logger.info(
      { userId: userId.toString(), tier, billingCycle },
      'Platform subscription created'
    );

    return authorAccountMapper.toPlatformSubscriptionDTO(updated);
  },
  ...
  async changePlan(
    ...
    const upgrading = isUpgrade(account.platformTier, newTier);
   ...
   * Cancel subscription
   */
  async cancel(userId: bigint): Promise<PlatformSubscriptionDTO> {
    const account = await authorAccountRepository.findByUserId(userId);
    if (!account) {
      throw new NotFoundError(
        'Author account not found',
    ...
    logger.info(
      { userId: userId.toString(), tier: account.platformTier },
   ...
   */
  async reactivate(userId: bigint): Promise<PlatformSubscriptionDTO> {
    const account = await authorAccountRepository.findByUserId(userId);
    if (!account) {
      throw new NotFoundError(
        'Author account not found',
