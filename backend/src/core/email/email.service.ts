 ...
 */
const EMAIL_TEMPLATES: Record<EmailTemplate, { subject: string; getHtml: (data: Record<string, unknown>) => string }> = {
  'welcome': {
...
export const emailService = {
  ...
  async send(options: SendEmailOptions): Promise<SendEmailResult> {
    ...
    if (!config.email.enabled || !config.email.sendgridApiKey) {
      ...
      if (data.confirmationUrl) {
        logger.info(`🔗 Confirmation URL: ${data.confirmationUrl}`);
      }
    ...
    try {
      ...
      logger.info({ to, template, messageId }, 'Email sent successfully');
   ...
   */
  async sendPasswordChangedEmail(to: string): Promise<SendEmailResult> {
    return this.send({
      to,
      subject: 'Your password has been changed',
